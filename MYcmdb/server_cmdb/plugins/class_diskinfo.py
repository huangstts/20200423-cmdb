    ...
    def parse(self,data):
        ...
        for line in data.split('\n'):
            ...
            if line.strip().startswith('physical id'):
                slot_num_li.append(line.strip().split(':')[-1].strip())
                # print(line.strip().split(':')[-1].strip())
            if line.strip().startswith('logical name'):
                pd_type_li.append(line.strip().split(':')[-1].strip())
                # print(line.strip().split(':')[-1].strip())
            if line.strip().startswith('size'):
                coerced_size.append(line.strip().split(':')[-1].split()[0])
                # print(line.strip().split(':')[-1].split()[0])
                # print(line.strip().split(':')[-1].split()[-1].strip('(').strip(')'))
                raw_size.append(line.strip().split(':')[-1].split()[-1].strip('(').strip(')'))
